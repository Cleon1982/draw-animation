...
function initialization(objFile){
  ...
  $('.infoText').text('操作说明：/nA. 更换其它图像。B. 放大或者缩小图像！然后，你可以靠近某个骨骼点，然后1. 按住鼠标左键不松，拖拽你的图画!或者2. 鼠标右键取消某个骨骼点！或者3.继续鼠标左键选择其它骨骼点！');
      ...
      if(leftClicked){
        ...
        if(barycentricCoordMode){
          ...
          if(handleExistsBaryCentricMode(nearestHandleIndex)){
            ...
            $('.infoText').text('Compilation started! 正在设置骨骼点！请稍后..');
          ...
          if(handleExists(nearestVertexIndex)){
            ...
            if(nearestVertexIndex != null){
              ...
              handles.push(newHandle);
              $('.infoText').text('正在设置骨骼点！请稍等..');
              setTimeout( function(){ compilation(handles,originalVertices,barycentricCoordMode,
                function(){ 
                  $('.infoText').text('操作完成！你可以靠近某个骨骼点，然后1. 按住鼠标左键不松，拖拽你的图画!或者2. 鼠标右键取消某个骨骼点！或者3.继续鼠标左键选择其它骨骼点！');
                  drawHandle(newHandle); 
                  }); 
                }, 20);
            }else{
  ...
  $('.boxObj').click(function(event) {
    localStorage.setItem("model", 'models/cjl.obj');
    location.reload();
  });
  
  $('.manObj').click(function(event) {
    localStorage.setItem("model", 'models/cby.obj');
    location.reload();
  });
  ...
  renderer.render(scene, camera);
	...
	renderer.setClearColor('rgba(255,255,2255,0.5)',1.0); //设置场景背景为白色
...
function loadObj(objPath){
  ...
  loader.load(
    ...
    function ( object ) {
     ...
     var texture = new THREE.TextureLoader().load(newstr)//加载纹理图片
